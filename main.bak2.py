...
#PATHFINDING_LOG.setLevel(logging.DEBUG)
...
def update_maze_block():
    ...
    while True:
        ...
        #new_maze = maze.copy()      # copy happens between update_maze
        row = maze[y_block-1]
...
def ghost_handler(ghost, init_old_symbol):
    init_old_x, init_old_y = ghost.x, ghost.y
    init = True
    ...
    while True:
        ...
        if old_symbol != False:
            ...
            if init == True:
                update_block(init_old_x, init_old_y, init_old_symbol)
                init = False
            else:
                update_block(old_x, old_y, old_symbol)
...
def find_shortest_way(entity, starting_pos, ending_pos):
    ...
    if starting_pos != ending_pos:
        ...
        shortest_way_steps = min(hits)      # if multiple ways with same steps it will take the first one
